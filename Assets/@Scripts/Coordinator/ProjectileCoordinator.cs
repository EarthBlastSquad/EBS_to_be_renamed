...
namespace Coordinator
{
    public class ProjectileCoordinator : MonoBehaviour
    {
        ...
        public void Init(SkillData data, Vector2Int gridCnt, Vector3 startPos, Vector3 endPos,IReadOnlyList<VictimCoordinator> victims , AttackManager attackMgr, float delayTime)
        {
            ...
            _actor.Init(Managers.Instance.ResourceManager.Load<AnimatorOverrideController>(data.SkillAnimationControllerName), data.SkillLaunchParticleName, data.SkillBOOMParticleName);//이부분은 런타임 성능이 너무 떨어진다 싶으면 그때 캐싱으로 바꿔보죠
            _accumulatedTime = 0;
